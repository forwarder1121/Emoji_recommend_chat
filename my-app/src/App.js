...
const App = () => {
    ...
    const API_URL =
        "https://api-inference.huggingface.co/models/forwarder1121/results";
    const headers = {
        Authorization: "Bearer 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",
    };
    ...
    const handleSubmit = (event) => {
        ...
        if (socket && message.trim()) {
            socket.send(
                JSON.stringify({ sender: username, message: message.trim() })
            );
